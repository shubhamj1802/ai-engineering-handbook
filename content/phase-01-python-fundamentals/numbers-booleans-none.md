...
Three separate surprises live in this lesson, and all three catch people out:

1. **Decimals are approximations.** `0.1 + 0.2` is not `0.3`.
2. **`None` is not zero.** It means "no value at all".
3. **`and` / `or` return one of your values**, not `True` or `False`.

<figure class="lesson-figure">
<svg viewBox="0 0 660 240" role="img" aria-label="Diagram: 0.1 plus 0.2 stored as binary floats produces 0.30000000000000004, so comparing it directly to 0.3 is false. Comparing with a tolerance, or using the decimal module for money, is correct.">
...
<figcaption>
<strong>This is not a Python bug.</strong> It happens in every language that stores decimals
in binary. Use <code>math.isclose</code> for measurements, and <code>Decimal</code> whenever
the number is money.
</figcaption>
</figure>

The other two surprises, quickly:

```python
# None means "nothing here" - which is NOT the same as 0 or ""
count = None
if count is None:          # use `is`, not ==
    count = 0

# and / or hand back one of your actual values
name = user_name or "anonymous"     # if user_name is empty, use the fallback
print(0 or "fallback")              # "fallback"  <- 0 counts as falsy!
```

:::warning Which values count as "false"
`0`, `0.0`, `""`, `[]`, `{}`, `None` and `False` are all treated as false. Everything else
is true.

That is handy, and occasionally a trap: `if count:` skips a real count of **zero**. When zero
is a legitimate value, write `if count is not None:` instead.
:::
